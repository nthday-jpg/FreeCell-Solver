...
# Fast path (assumes valid card codes)
def can_stack_on_cascade_code(moving_code: int, destination_top_code: int | None) -> bool:
    if destination_top_code is None:
        return True
    return CARD_CODE_IS_RED[moving_code] != CARD_CODE_IS_RED[destination_top_code] and \
           CARD_CODE_RANK[moving_code] + 1 == CARD_CODE_RANK[destination_top_code]

def is_descending_alternating_codes(card_codes: tuple[int, ...]) -> bool:
    if len(card_codes) <= 1:
        return True
    for lower_code, upper_code in zip(card_codes, card_codes[1:]):
        if CARD_CODE_RANK[lower_code] != CARD_CODE_RANK[upper_code] + 1 or \
           CARD_CODE_IS_RED[lower_code] == CARD_CODE_IS_RED[upper_code]:
            return False
    return True
