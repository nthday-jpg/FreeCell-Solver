...
def benchmark_ucs_expansions(seed_value: int, target_expansions: int) -> SolveResult:
    cascades = deal_cascades(seed_value)
    init = GameState(cascades).to_packed()
    solver = UCSSolver(max_expansions=target_expansions)
    return solver.solve(init)


def main() -> None:
    start_seed = 10
    target_expansions = 100000
    warmup_runs = 0
    measured_runs = 10
    current_seed = start_seed

    for run_index in range(1, warmup_runs + 1):
        result = benchmark_ucs_expansions(seed_value=current_seed, target_expansions=target_expansions)
        print(
